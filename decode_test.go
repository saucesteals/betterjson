...
var unmarshalTests = []unmarshalTest{
	...
	},
	{
		...
		err: fmt.Errorf("json: invalid use of ,string struct tag, trying to unmarshal %q into betterjson.Number", `invalid`),
	},
	{
		in:  `{"A":"invalid"}`,
...
func TestUnmarshalEmbeddedUnexported(t *testing.T) {
	...
	tests := []struct {
		...
		ptr: new(S1),
		out: &S1{R: 2},
		err: fmt.Errorf("json: cannot set embedded pointer to unexported struct: betterjson.embed1"),
	}, {
		...
		ptr: new(S5),
		out: &S5{R: 2},
		err: fmt.Errorf("json: cannot set embedded pointer to unexported struct: betterjson.embed3"),
	}, {
